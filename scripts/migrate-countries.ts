...
async function main() {
  ...
  console.log(`Found ${agents.length} agents with country set`)

  let updated = 0
  let skipped = 0

  for (const agent of agents) {
    const from = agent.country!
    const to   = NATIONALITY_TO_COUNTRY[from]
    if (!to || to === from) { skipped++; continue }

    await db.agent.update({ where: { id: agent.id }, data: { country: to } })
    console.log(`  ${from} → ${to}`)
    updated++
  }

  console.log(`\nDone: ${updated} updated, ${skipped} skipped (already country name or no mapping)`)
}
