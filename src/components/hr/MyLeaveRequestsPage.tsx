...
import { Plus, X, CalendarDays, ClipboardList, ShieldAlert } from 'lucide-react'
...
export default function MyLeaveRequestsPage() {
  ...
  function openAdd() { setForm({ startDate: '', endDate: '', reason: '' }); setFormError(''); setModal(true) }
          ...
          <tbody className="divide-y">
            {requests.length === 0 ? (
              ...
              <tr key={r.id} className="hover:bg-muted/30 transition-colors">
                ...
                <td className="px-4 py-3 text-muted-foreground text-xs max-w-48 truncate" title={r.decisionNote ?? undefined}>{r.decisionNote ?? '—'}</td>
              </tr>
            ))}
          </tbody>
        </table></div>
