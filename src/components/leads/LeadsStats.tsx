...
import { Users, Globe, Target, Clock, Flag, TrendingUp, ArrowUp, ArrowDown, X } from 'lucide-react'
...
export default function LeadsStats() {
  ...
  const fetchStats = useCallback(async () => {
    setLoading(true)
    try {
      const curReq  = fetch(`/api/leads/stats?from=${activeFrom}&to=${activeTo}`).then(r => r.ok ? r.json() : null)
      const prevReq = compare ? fetch(`/api/leads/stats?from=${prevR.from}&to=${prevR.to}`).then(r => r.ok ? r.json() : null) : Promise.resolve(null)
  ...
  // Aligned by day-offset (not calendar date) so two equal-length windows overlay.
          ...
          {loading ? (
            ...
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={dailyChart} margin={{ top: 4, right: 8, left: -10, bottom: 0 }}>
                ...
                <XAxis dataKey="idx" tickFormatter={v => `Day ${v}`} tick={{ fontSize: 10 }} axisLine={false} tickLine={false} minTickGap={20} />
                <YAxis allowDecimals={false} tick={{ fontSize: 11 }} axisLine={false} tickLine={false} />
                <Tooltip content={<SeriesTooltip formatLabel={l => `Day ${l}`} />} />
                ...
                <Line type="monotone" dataKey="previous" name="Previous period" stroke={MUTED} strokeWidth={2} strokeDasharray="4 4" dot={false} activeDot={{ r: 4 }} />
              </LineChart>
            </ResponsiveContainer>
          ) : (
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={data.daily} margin={{ top: 4, right: 8, left: -10, bottom: 0 }}>
                ...
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e5e7eb" />
                <XAxis dataKey="date" tickFormatter={fmtDate} tick={{ fontSize: 10 }} axisLine={false} tickLine={false} minTickGap={24} />
                <YAxis allowDecimals={false} tick={{ fontSize: 11 }} axisLine={false} tickLine={false} />
          ...
          {loading ? (
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={hourlyChart} barCategoryGap="20%" margin={{ top: 4, right: 8, left: -10, bottom: 0 }}>
                ...
                <Bar dataKey="previous" name="Previous period" fill={MUTED} radius={[4, 4, 0, 0]} maxBarSize={14} />
              </BarChart>
            </ResponsiveContainer>
          ) : (
            <ResponsiveContainer width="100%" height={200}>
