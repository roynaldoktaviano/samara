...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (action === 'generate_invoice') {
      ...
      const { amount, paymentType, paymentMethod, notes, billToType, bankId, paymentLink, changeProofNote, changeProofUrl } = body
      ...
      await withRetry(db, () => db.payment.update({
        ...
        data: {
          ...
          // First generation moves the payment from "requested" to "invoice_ready" for Sales to
          // act on. Editing an already-generated invoice no longer resets status or clears an
          // already-submitted proof — Finance instead attaches optional changeProofUrl evidence
          // below and Sales is just notified, so Confirm Payment stays available right away.
          ...(isFirstGeneration ? { status: 'invoice_ready' } : {}),
          ...(changeProofNote !== undefined && { changeProofNote: changeProofNote || null }),
          ...(changeProofUrl !== undefined && { changeProofUrl: changeProofUrl || null }),
        },
      }))
      ...
      if (!isSilent) {
        ...
        const notifBody = isFirstGeneration
          ...
          : `${invoiceNumber} (${existing.booking.bookingCode}) was updated by Finance — please review the change${changeProofUrl ? ' (proof attached)' : ''}`
