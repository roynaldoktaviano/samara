...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string; followUpId: string }> }) {
  ...
  const existing = await db.purchaseRequestFollowUp.findUnique({ where: { id: followUpId }, select: { requestId: true } })
  if (!existing || existing.requestId !== id) return NextResponse.json({ error: 'Not found' }, { status: 404 })

  await db.purchaseRequestFollowUp.delete({ where: { id: followUpId } })
