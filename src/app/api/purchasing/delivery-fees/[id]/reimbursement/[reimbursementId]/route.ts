...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string; reimbursementId: string }> }) {
  ...
  const result = await db.deliveryFeeReimbursement.updateMany({
    where: { id: reimbursementId, deliveryFeeId: id },
    data: {
  ...
  const updated = await db.deliveryFeeReimbursement.findUnique({ where: { id: reimbursementId } })
