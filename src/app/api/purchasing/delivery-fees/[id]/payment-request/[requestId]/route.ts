...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string; requestId: string }> }) {
  ...
  const result = await db.deliveryFeePaymentRequest.updateMany({
    where: { id: requestId, deliveryFeeId: id },
    data: {
  ...
  const updated = await db.deliveryFeePaymentRequest.findUnique({ where: { id: requestId } })
